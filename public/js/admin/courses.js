...
$(document).on('click', '.open-course-info-modal', function () {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      $('#course-info-instructor').attr(
        'href',
        `/users/${result.data.instructor._id}`
      );
      $('#course-info-slug').text(`${result.data.slug}`);
      $('#course-info-views').text(`${result.data.views}`);
      $('#course-info-brief-description').text(
        `${result.data.briefDescription}`
      );
      $('#course-info-detail-description')
        .html(`${result.data.detailDescription}`)
        .contents();
      $('#course-info-price').text(`${result.data.price}`);
      $('#course-info-category').text(`${result.data.subCategory.name}`);
      $('#course-info-updatedAt').text(
